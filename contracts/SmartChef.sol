...
// added functionnalities by the paddyswap finance's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when SROCK mining ends.
    uint256 public bonusEndBlock;
    // Deposited amount SROCK in SmartChef
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = depositedSrock;
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 srockReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Stake SROCK tokens to TheBushV1
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if(pool.depositFeeBP > 0){
                ...
                depositedSrock = depositedSrock.add(_amount).sub(depositFee);
            }else{
                user.amount = user.amount.add(_amount);
